...
> **Note for this project:** GoghMode has no routes, no accounts, and no
> permissions, so the `Auth & permissions` sections of the templates are answered
> with "none, and why". The **Estimate** tables mark shipped scope as `shipped` —
> [PLANNING.md](../PLANNING.md) deliberately carries no dates or estimates.
...
### Components
- [x] [export-contract](components/export-contract.md) — Default · Empty · Out-of-bounds · Write failure
- [x] [mobile-server-api](components/mobile-server-api.md) — Running · Port fallback · Start failed · Rejected · Write failure · Closing
