...
- Pairing is a single-use code with a roughly 120-second lifetime, shown as a QR code, and **a
  person taps approve on the host** before any secret is issued. The host hands back a per-device
  secret, recorded in `~/.goghmode/devices.json` mode `0600`.
- Every upload carries `HMAC-SHA256(deviceSecret, deviceId ‖ timestamp ‖ nonce ‖ hostId ‖
  ...
  seconds, the nonce is unseen in that window, and the signature matches — then answers a single
  generic `401` for any failure.
...
- Sniffing the network no longer yields a working credential, because the secret does not travel
  after pairing.
